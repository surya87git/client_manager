...
class Welcome extends CI_Controller {
	...
	public function ajax_client_review()
	{
		...
		if($booking_id != ""){
			...
			$res = $this->Master_model->updateArr("bkf_booking_transaction", $frm_trans, $where_arr);
	...
	public function client_booking_pdf()
	{
		...
		// $qry = "SELECT * FROM tbl_cost_calculator_new where id = $data['client_info'][0]->calc_id";
        // $data['work_area'] = $this->Master_model->getCustom($qry);


        $file_name = $data['client_info'][0]->mobile_no.'_'.time().'.pdf';

        // echo $this->load->view('client_booking_pdf',$data,true);
        // exit;
