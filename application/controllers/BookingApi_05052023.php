			...
			if($booking_id == "" )
			{
				...
				{			
					...
					if($booking_id)
					{
						...
						$this->Master_model->saveData("bkf_booking_form", $new_data);
...
/***--------Start Dashboard--------------------------------------*/
	...
	public function dashboard()
	{
		...
		$qry = "SELECT SUM(booking_amt) AS bamt , COUNT(id) AS bkng_total  FROM `bkf_booking_form` WHERE `admin_approval` = 1 ";
		$data['booking_amt'] = $this->Master_model->getCustom($qry);

		// $qry = "SELECT SUM(booking_amt) AS bamt FROM `bkf_booking_form` WHERE `admin_approval` = 1 ";
		// $data['booking_amt'] = $this->Master_model->getCustom($qry);
