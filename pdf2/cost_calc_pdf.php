...
if($res)
{
  ...
  $html .='<tr><td>Date:- '.$res["create_date"].'</td><td>Create By:- '.$res["create_by"].'</td></tr>';
  ...
  $html .='<tr><td colspan="2"><span style="font-size: 18px; font-weight:bold;">Number of Floor:- '.$res['floor_num'].'</span></td></tr>';
    ...
    {  
        ...
        $html .='<table cellspacing="0" class="table table-bordered" style="background-color: #FEE; font-size: 16;">';
        $html .= '<tr align="left"><th>Floor</th><th>Area</th><th>Rate</th></tr>';
        $cnt = 0;
        while($res1 = mysqli_fetch_array($exec1))
        {
            $html .='<tr><td align="left">'.$res1['floor'].'</td><td>'.number_format($wa_arr[$cnt]).'</td><td align="left">₹ '.number_format($rate_arr[$cnt]).'</td></tr>';
            $cnt++;  
        }
        $html .= '<tr><th align="left">Total</th><th align="left">'.number_format($res['total_work_area']).' sqft</th><th align="left">₹ '.number_format($res['floor_tsq_price']).'</th></tr>';
        
        $html .= '</table>';
        $html .= '</td></tr>';
    }
    
    $html .='<tr><td colspan="2"><span style="font-size: 16px; font-weight:bold;">Total Floor Price:- ₹ '.number_format($res['sub_total']).'</span></td></tr>';
