...
$followup_by_id = $_REQUEST["fid"] ?? ""; 
$create_by_id = $_REQUEST["cid"] ?? ""; 
...
$qry ="";
$qry .="SELECT * FROM tbl_followup_master where is_private = 'NO'";
if($followup_by_id)
{
...
if(isset($stmt))
{
  ...
  while($res = mysqli_fetch_array($stmt))
  {
    ...
    $team_work = $res["team_work"] ?? '';
    $emp_name = "";
    if($team_work != "")
    {
        $team_work = explode(",", $res["team_work"]);
        //print_r($team_work);
        foreach($team_work as $id)
        {
    ...
    $html .='<table cellspacing="0" class="table table-bordered" style="background-color: #FEE; font-size: 16;">';
      ...
      $html .='<tr><td colspan="2">Team Work:- '.$emp_name.'</td></tr>';
